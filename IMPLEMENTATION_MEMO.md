...
- **変更点**:
  - フレンドページから「冒険の招待」ボタンを削除
  - `/adventure/invites` にアクセスした場合は `/adventure` へリダイレクト
- **残存**: パーティーモードの招待（`/party/invites`）は従来どおり利用可能
...
2. `supabase_party_invites.sql` を実行（フレンド招待する場合）
3. トップまたはゲームページから「パーティーモード」へ
4. パーティー編成（最大3体）→「誘う」でフレンド招待 または 「ステージを選ぶ」でソロプレイ
5. フレンドは「パーティの招待」で参加（フレンドページ→パーティの招待、またはパーティページ→招待を見る）
